...
launcher_template="$project_dir/scripts/run_server.sh"
launcher="$artifact_dir/run_server.sh"
binary="$project_dir/target/release/telemetry-server"
...
echo "Building telemetry-server release binary..."
"$cargo_bin" build --locked --release -p telemetry-server
...
cat >"$unit_tmp" <<EOF
[Unit]
Description=CC Switch Telemetry Server
